...
Okay, I started working on another script to get the state GDP per
capita. It’s a little more straightforward than the demographic data
(since [FRED](https://fred.stlouisfed.org/) has way more consistent data
formats than the census bureau), so I’ll just drop the final script into
a code chunk when I’m done.

One thing that’s causing me to scratch my head a bit is that the data on
FRED for total state GDP (at least that I have now) only goes back to
1997. I’m not sure what to do about that… I need to read up on whether
or not I can impute the previous data (probably not), only utilize this
predictor for years when it’s available (maybe? though I think that
would also throw off the regression for any pre-1997 elections), merge
pretty much every race together (maybe can do this, though I’m not sure
the literature supports this - would mean treating Texas exactly the
same as California, for example.), or dig thru & find more data (pre
1997) (ideal…).

Anyway, I’ll deal with that later, for now here’s a quick chart showing
the real gdp per capita of each state. There’s a quick drop at 2000
becasue projected population growth was updated to actuals with the 2000
census. The big outlier is D.C.
...
Hmmmm this also makes me think - maybe I ought to scale the predictor on
a log10 scale as well… Something to think about once I get to ghat
point.

### State Unemployment

Similar to GDP per capita, the unemployment rate is pretty simple (see
wrangle\_unemployment.R). Here’s how it looks historically:
...
Okay so this definitely isn’t an economic indicator, but I figure it’s
good to add in. [This
dataset](https://www.ers.usda.gov/webdocs/DataFiles/48747/Education.xls)
gives the educational attainment by state from census bureau surveys
since 1970. I’ll probably have to interpolate between years, but that
should be okay.
